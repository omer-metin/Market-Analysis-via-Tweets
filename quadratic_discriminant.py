...
RESULT_FILE_NAME = time.strftime("quadratic_discriminant_test_%y-%m-%d_%H-%M-%S.csv",
...
if not THRESH_TEST:
    ...
    for n in PARAMETERS:
        ...
        clf = QuadraticDiscriminantAnalysis(**n)
    ...
    for thres in THRESHOLDS:
        ...
        clf = QuadraticDiscriminantAnalysis(**PARAMETER)
        t0 = time.time()
        clf.fit(train_X, train_y)           # training
        dT = time.time() - t0
        test_results = clf.predict(test_X)  # test
