...
DATA_FRAC = 0.1             # Ratio that determines used data in process

THRESHOLDS = np.linspace(0.1, 2.0, 10)   # Works if C_GAMMA_TEST False

WRITE_FILE = True          # Write results to file
RESULT_FILE_NAME = time.strftime("guassian_process_test_%y-%m-%d_%H-%M-%S.csv",
...
if not THRESH_TEST:
    ...
    if WRITE_FILE:
        ...
        result_file.write(f"Method: Guassian Process\n")
    ...
    print("starting...")
    for n in PARAMETERS:
        clf = GaussianProcessClassifier(**n)
        ...
        if WRITE_FILE:
            result_file.write(f"0,{n},{acc:.5f},{dT:.2f}\n")
            result_file.flush()
        i += 1
    ...
    for thres in THRESHOLDS:
        ...
        clf = GaussianProcessClassifier(**PARAMETER)
        t0 = time.time()
        clf.fit(train_X, train_y)           # training
        dT = time.time() - t0
        test_results = clf.predict(test_X)  # test
