...
#--- VARIABLES ---#
CACHESIZE = 2000            # Cache size in MB
...
C_GAMMA_TEST = False        # C and Gamma Training or Threshold training
THRESHOLD = 0.5             # Company change threshold in %
C_VALUES = [1, 7]           # C values for SVM
GAMMA_VALUES = [1e-9, 1e-4]       # Gamma values for SVM

C_VALUE = 1                 # Works if C_GAMMA_TEST False
GAMMA_VALUE = 1e-9          # Works if C_GAMMA_TEST False
# np.linspace(0.1, 2.0, 10)   # Works if C_GAMMA_TEST False
THRESHOLDS = [0.4, 0.5, 0.7]

WRITE_FILE = True          # Write results to file
RESULT_FILE_NAME = time.strftime("svm_dbd_test_%y-%m-%d_%H-%M-%S.csv",
...
def get_data(threshold):
    ...
    for raw_tw in raw_tweet_data:
        ...
        if not cur_date in data_d:
            ...
            data_row.extend(raw_tw[3:])
            data_d[cur_date] = np.array(data_row, dtype=np.float32)
        else:
            data_d[cur_date][1:] += np.array(raw_tw[3:], dtype=np.float32)
    print(pos, neg, net)
    res_data = []
