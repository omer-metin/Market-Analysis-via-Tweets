...
RESULT_FILE_NAME = time.strftime("adaboost_test_%y-%m-%d_%H-%M-%S.csv",
...
PARAMETERS = [{'n_estimators': n} for n in range(50, 1050, 50)]
...
if not THRESH_TEST:
    ...
    if WRITE_FILE:
        ...
        result_file.write(f"Method: ADA Boost\n")
    ...
    for thres in THRESHOLDS:
        ...
        clf = AdaBoostClassifier(**PARAMETER)
        t0 = time.time()
        clf.fit(train_X, train_y)           # training
        dT = time.time() - t0
        test_results = clf.predict(test_X)  # test
